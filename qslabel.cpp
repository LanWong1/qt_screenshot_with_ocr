...
using namespace Qt;
using namespace cv;
bool sortResults(vector<Point2f> result1, vector<Point2f> result2 ){
    return result1[0].y < result2[0].y;
}
...
QSLabel::QSLabel(QWidget* parent)
{
    ...
    QStringList list = appPath.split(u'/');
    for(int i=0;i<list.count();i++){
        ...
        QString subfix = ".exe";
#endif
        if(list[i].contains( subfix) ){
            break;
        }
        rootPath.append(list[i]);
    }
    QString path =   rootPath.join(u"/");
    string std_rootPath = path.toStdString();


    detect_model = new TextDetector(std_rootPath);
    rec_model = new TextRecognizer(std_rootPath);
    angle_model = new TextClassifier(std_rootPath);
}


void QSLabel::ocr(Mat inputMat)
{
    vector<OCRPredictResult> res_tmp;
    float scale = float(geometry().width()) / inputMat.cols;
    vector< vector<Point2f> > results = detect_model->detect(inputMat);
    if(results.size()==0){
        qDebug() << "no txt found";
        ocr_results = res_tmp;
        ocrDone = true;
        ...
        detect_model->draw_pred(inputMat, results);
    //    imshow("1", inputMat);
    //    waitKey(0);
        //Mat reScaleImage;
        //cv::resize(inputMat,reScaleImage, Size(0,0), scale,scale, INTER_LINEAR);
    //    selectimage = cvMat2QImage( reScaleImage );
    //    update();
        for (size_t i = 0; i < results.size(); i++)
        {
            ...
            QT_TRY {
                ...
                textimg = detect_model->GetRotateCropImage(inputMat, results[i]);
                ocrBoxList.append(results[i]);
                string text = rec_model->predict_text(textimg);
                res.box = results[i];
                res.text = text;
                res_tmp.push_back(res);
                //ocrTextList.append(QString::fromStdString(text));
            }
            QT_CATCH(exception) {
                qDebug() << "get rotate crop failed"<<endl;
                continue;
            }
        }
        sorted_boxes(res_tmp);
    }
...
QList<vector<Point2f> > QSLabel::getOcrBoxes()
{
    return this->ocrBoxList;
}

vector<OCRPredictResult> QSLabel::getOcrResults()
{
    return this->ocr_results;
}

QList<QString> QSLabel::getOcrTexts()
{

    if(ocr_results.size()==0){
        qDebug() << "we got no txt";
        ocrTextList.append("");
    }

    else{
        std::string txt =ocr_results[0].text;
        for (int i = 0; i < ocr_results.size() - 1; i++) {
            float h1 = (ocr_results[i].box[0].y + ocr_results[i].box[3].y) / 2;
            float h2 = (ocr_results[i+1].box[0].y + ocr_results[i+1].box[3].y) / 2;
            if ((abs(h1 - h2)) > 10){
                std::string temp = txt + "\n";
                //ocrTextList.push_back(temp);
                ocrTextList.append(QString::fromStdString(temp));
                txt = ocr_results[i+1].text;
            }
            else{
                txt += ocr_results[i+1].text;
            }
            if (i == ocr_results.size() - 2)
                ocrTextList.append(QString::fromStdString(txt));
        }
    }
...
void QSLabel::showEvent(QShowEvent *event)
{
    QImage res = this->resultimage();
    this->ocr(this->qim2mat(res));
}
...
bool QSLabel::getDrawStatus()
{
    qDebug() << isdrawline << isdrawrectangle << isdrawround << istextedit << isdrawarrow << isOcr;
    return isdrawline || isdrawrectangle || isdrawround || istextedit|| isdrawarrow || isOcr;
}

bool QSLabel::getOCRStatus()
{
    return ocrDone;
}
