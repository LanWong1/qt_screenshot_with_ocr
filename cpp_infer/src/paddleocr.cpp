...
PPOCR::ocr(std::vector<cv::Mat> cv_all_img, bool det, bool rec,
           bool cls) {
  ...
  if (!det) {
    ...
    for (int i = 0; i < cv_all_img.size(); ++i) {
      cv::Mat srcimg =cv_all_img[i];// cv::imread(cv_all_img_names[i], cv::IMREAD_COLOR);
      if (!srcimg.data) {
          continue;
...
//        exit(1);
    ...
    for (int i = 0; i < cv_all_img.size(); ++i) {
      std::vector<OCRPredictResult> ocr_result;
      cv::Mat srcimg =cv_all_img[i]; // cv::imread(cv_all_img[i], cv::IMREAD_COLOR);
      if (!srcimg.data) {
          continue;
//        std::cerr << "[ERROR] image read failed! image path: "
